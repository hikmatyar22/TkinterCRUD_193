...
def fetch_data():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM nilai_siswa")  # Mengambil seluruh data
    rows = cursor.fetchall()  # Menyimpan hasil query ke dalam list
    conn.close()
    return rows
...
def save_to_database(nama, biologi, fisika, inggris, prediksi):
    conn = sqlite3.connect('nilai_siswa.db')
    cursor = conn.cursor()
    cursor.execute('''  # Menyimpan data ke tabel
...
def update_database(record_id, nama, biologi, fisika, inggris, prediksi):
    conn = sqlite3.connect('nilai_siswa.db')
    cursor = conn.cursor()
    cursor.execute('''  # Mengupdate data berdasarkan ID
        UPDATE nilai_siswa
...
def populate_table():
    for row in tree.get_children():
        tree.delete(row)  # Menghapus baris lama
    for row in fetch_data():
        tree.insert('', 'end', values=row)  # Menambahkan baris baru dari database

# Fungsi untuk mengisi input dari data yang dipilih di tabel
def fill_inputs_from_table(event):
    try:
        selected_item = tree.selection()[0]  # Mendapatkan baris yang dipilih
...
tree.bind('<ButtonRelease-1>', fill_inputs_from_table)  # Mengisi form berdasarkan data yang dipilih
